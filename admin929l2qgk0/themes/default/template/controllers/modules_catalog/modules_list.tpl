            ...
            <div class="panel">
                <div class="module-info-wrapper">
                    ...
                    <div class="panel-action clearfix">
                        <span class="module-price">{if isset($element->price)}{if $element->price|floatval == 0}{l s='Gratuit'}{else}{$element->price_formatted}{/if}{else}{l s='Gratuit'}{/if}</span>
                        {if isset($element->type) && $element->type == 'addonsMustHave' && !$element->not_on_disk}&nbsp;<span class="label label-primary">{l s='Dans le répertoire des modules'}</span>{/if}
                        ...
                        {else}
                            <a class="btn button-action pull-right btn-primary{if !$element->trusted} untrustedaddon{/if}" {if !$element->trusted} data-target="#moduleNotTrusted"  data-toggle="modal"{/if} data-link="{$element->options.install_url|escape:'html':'UTF-8'}" data-module-display-name="{$element->displayName|escape:'html':'UTF-8'}" data-module-name="{$element->name|escape:'html':'UTF-8'}" data-module-image="{if isset($element->image)}{$element->image}{else}{$modules_uri}/{$element->name}/{$element->logo}{/if}" data-author-name="{$element->author|escape:'html':'UTF-8'}" data-author-uri="{if isset($element->author_uri)}{$element->author_uri|escape:'html':'UTF-8'}{/if}" href="{if !$element->trusted}#{else}{$element->options.install_url|escape:'html':'UTF-8'}{/if}" title="{l s='Installer'}">{l s='Installer'}</a>
                        {/if}
                    </div>
                </div>
            </div>
