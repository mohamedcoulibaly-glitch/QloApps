...
<div class="alert alert-info">{l s='Choisissez le nouveau statut pour votre commande'}</div>
<div class="form-horizontal">
	<input type="hidden" name="id_supply_order" id="id_supply_order" value="{$supply_order->id}">
	<div class="form-group">
		<label class="control-label col-lg-3">{l s='Statut de la commande :'}</label>
		<div class="col-lg-9">
...
<input type="submit" id="{$table}_form_submit_btn" value="{l s='Enregistrer'}" name="submitChangestate" class="button" style="display: none;">
</div>
{/if}
{/block}
...
<div class="panel">
	<h3><i class="icon-download-alt"></i> {l s='Imprimer le formulaire de commande d&#039;approvisionnement'}</h3>
	<a href="{$link->getAdminLink('AdminPdf')|escape:'html':'UTF-8'}&amp;submitAction=generateSupplyOrderFormPDF&amp;id_supply_order={$supply_order->id|intval}" onclick="return !window.open(this.href);" title="Export as PDF" class="btn btn-default"><i class="icon-download-alt"></i> {l s='Cliquez ici pour télécharger le formulaire de commande d&#039;approvisionnement.'}.</a>
</div>
{/if}
