...
<div class="row">
	<div class="col-lg-6">
		<div class="panel">
			<h3><i class="icon-user"></i> {l s='Informations client'}</h3>
			{if $customer->id}
				...
				<div class="form-horizontal">
					<div class="form-group">
						<label class="col-lg-3 control-label">{l s='Date d&#039;inscription :'}</label>
						<div class="col-lg-3"><p class="form-control-static">{dateFormat date=$customer->date_add}</p></div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">{l s='Commandes valides passées :'}</label>
						<div class="col-lg-3"><p class="form-control-static">{$customer_stats.nb_orders}</p></div>
					</div>
					<div class="form-group">
						<label class="col-lg-3 control-label">{l s='Total dépensé depuis l&#039;inscription :'}</label>
			...
			{else}
				<h2>{l s='Client non enregistré.'}</h2>
			{/if}
		</div>
	</div>
	<div class="col-lg-6">
		<div class="panel">
			<h3><i class="icon-shopping-cart"></i> {l s='Informations de(s) commande(s)'}</h3>
			{if is_array($cart_orders) && count($cart_orders)}
				...
				{/foreach}
				{l s='Créé le :'} {dateFormat date=$cart_order.date_add}
			{else}
				<h2>{l s='Aucune commande n&#039;a été créée depuis ce panier.'}</h2>
				{if $customer->id && !$hotel_products && !$standalone_products}
					<a class="btn btn-default" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&amp;cart_id={$cart->id|intval}&amp;addorder"><i class="icon-shopping-cart"></i> {l s='Créer une commande depuis ce panier.'}</a>
				{/if}
			{/if}
		</div>
	</div>
...
<div class="panel">
	<h3><i class="icon-archive"></i> {l s='Récapitulatif du panier'}</h3>
	<div class="row">
		<table class="table" id="orderProducts">
			<thead>
				<tr>
					...
					<th><span class="title_box">{l s='Hôtel'}</span></th>
					<th><span class="title_box">{l s='Durée'}</span></th>
					<th><span class="title_box">{l s='occupancy'}</span></th>
					<th><span class="title_box">{l s='Prix de la chambre'}</span></th>
					<th><span class="title_box">{l s='Services supplémentaires'}</span></th>
			...
			<tbody>
				{foreach from=$cart_htl_data item='room'}
					<tr>
						...
						<td>
							<span>
								{if $room['adults']}{$room['adults']}{/if} {if $room['adults'] > 1}{l s='Adultes'}{else}{l s='Adulte'}{/if}{if {$room['children']}}, {$room['children']} {if $room['children'] > 1}{l s='Enfants'}{else}{l s='Enfant'}{/if}{/if}
							</span>
						</td>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-body" id="rooms_extra_demands">
									...
									<ul class="nav nav-tabs" role="tablist">
										{if isset($room['selected_demands']) && $room['selected_demands']}
											<li role="presentation" class="active"><a href="#room_type_demands_desc_{$room['id']}" aria-controls="facilities" role="tab" data-toggle="tab">{l s='Équipements'}</a></li>
										{/if}
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-secondary" data-dismiss="modal">{l s='Fermer'}</button>
								</div>
							</div>
						</div>
					</div>
			...
			<tfoot>
				...
				<tr>
					<td colspan="7">{l s='Coût total de livraison :'}</td>
	...
	<div class="row">
		<table class="table">
			<thead>
				<tr>
					<th class="fixed-width-xs"><img src="../img/admin/coupon.gif" alt="{l s='Remises'}" /></th>
					<th>{l s='Nom de la remise'}</th>
					<th class="text-right fixed-width-md">{l s='Valeur'}</th>
				</tr>
			</thead>
			<tbody>
			{foreach from=$discounts item='discount'}
				<tr>
					...
					<td class="text-right fixed-width-md">{if (float)$discount.value_real == 0 && (int)$discount.free_shipping == 1}{l s='Livraison gratuite'}{else}- {displayWtPriceWithCurrency price=$discount.value_real currency=$currency}{/if}</td>
				</tr>
			{/foreach}
			</tbody>
		</table>
	...
	<div class="row alert alert-warning">
		{l s='For this particular customer group, prices are displayed as:'} <b>{if $tax_calculation_method == $smarty.const.PS_TAX_EXC}{l s='HT'}{else}{l s='TTC'}{/if}</b>
	</div>
{/block}
</div>
